							...
							<tbody>
								@foreach($data as $value)
									<tr class="text-center">
										...
										<td>{{$value->bankmember->account_name}}</td>
										<td>{{$value->total}}</td>
										<td>{{$value->fee}}</td>
										<td>{{$value->note}}</td>
										<td>
                                            ...
                                            @endif
                                         	@if($value->states == -1)
                                                {!!"<span class=\"label label-danger status\">Từ chối</span>"!!}
                                            @endif
										</td>
										<td>
											{{Form::model($value,['method'=>'PATCH','route'=>['adminwithdrawal.update',$value->id]])}}
												<input type="hidden" name="member_id" value="{{$value->member_id}}">
												<button name="status" value="a" title="Chấp nhận yêu cầu" type="submit" class="btn-primary btn btn-xs"><i class="fa fa-check"></i>Đã chuyển</button>
                                                <button name="status" value="b" title="Hủy yêu cầu" type="submit" class="btn-danger btn btn-xs"><i class="fa fa-times"></i>Từ chối</button>
											{{Form::close()}}
										</td>
										<td>
										{!! Form::open(['method'=>'DELETE','onsubmit'=>'return confirm("Bạn thật sự muốn xóa?")','route'=>['adminrecharge.destroy',$value->id]]) !!}
										{{-- <a href="adminwithdrawal/{{ $value->id }}" class="member-detail">Xem</a> --}}
